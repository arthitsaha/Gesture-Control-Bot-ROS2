...
class lychee(Node):
	...
	def detect_motion(sensitivity=(1/40),showjoint=False,joint_index=8):
		global direction
		x,y,_ = 0,0,0
		thehell = False
		t0 = time.time()
	...
	def __init__(self):
		...
		while cv2.waitKey(1) != ord('q'):
			...
			text1 = 'forward -> all fingers open '
			text2 = 'stop -> all fingers closed'
			text3 = 'left -> show 1'
			text4 = 'right -> show 2'
			text5 = 'back -> show 3'
			...
			frame = cv2.putText(frame, text1, position, font, font_scale, color, font_thickness, lineType=cv2.LINE_AA)
			frame = cv2.putText(frame, text2, (50, 80), font, font_scale, color, font_thickness, lineType=cv2.LINE_AA)
			frame = cv2.putText(frame, text3, (50, 110), font, font_scale, color, font_thickness, lineType=cv2.LINE_AA)
			frame = cv2.putText(frame, text4, (50, 140), font, font_scale, color, font_thickness, lineType=cv2.LINE_AA)
			frame = cv2.putText(frame, text5, (50, 170), font, font_scale, color, font_thickness, lineType=cv2.LINE_AA)
